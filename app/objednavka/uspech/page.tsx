...
export default async function OrderSuccess({ searchParams }: { searchParams: Promise<{ session_id?: string }> }) {
  ...
  return <main className="orderResult"><section className="resultCard">
    ...
    <Link className="primary" href={paid ? `/pruvodce?session_id=${encodeURIComponent(sessionId ?? "")}` : "/#objednat"}>{paid ? "Otevřít Wi‑Fi Doktora" : "Zkusit znovu"} <span aria-hidden="true">→</span></Link>
  </section></main>;
}
